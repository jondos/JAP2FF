...
function init(){
    try{
        ...
        if (userLevel>0){
            var warnOn = oPS.getBoolPref('extensions.jap2ff.onOffWarning');
            if (warnOn){
...
function changeWarning(inp){
   try{
       var oPS = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("");
       if (inp.checked){
           oPS.setBoolPref('extensions.jap2ff.onOffWarning',false);       
       }else{
           oPS.setBoolPref('extensions.jap2ff.onOffWarning',true);
       }
   }catch(e){
        _trace(e);   
   }
...
function doOK(){
    var oPS = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("");
    oPS.setBoolPref('extensions.jap2ff.onOffBack',true);
    return true;
}

function doCancel(){
    var oPS = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("");
    oPS.setBoolPref('extensions.jap2ff.onOffBack',false);
    return true;
}
